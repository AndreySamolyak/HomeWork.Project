...
def donuts(count):      # 2016.03.17_10:44:20 checked. prusanov
    # +++ ваш код +++
    if count < 10:
...
def both_ends(s):       # 2016.03.17_10:44:25 checked. prusanov
    # +++ ваш код +++
    if len(s) < 2:
        return ""
    else:
...
def fix_start(s):       # 2016.03.17_10:44:40 checked. prusanov
    # +++ ваш код +++
    if len(s) <= 1:
        return ""
    else:
...
def mix_up(a, b):       # 2016.03.17_10:44:47 checked. prusanov
    # +++ ваш код +++
...
# Этот ужин хорош!
def not_bad(s):     # 2016.03.17_10:44:59 checked. prusanov
    # +++ ваш код +++
    notindex = s.find('не')
    badindex = s.find('плох')
    if notindex != -1 and badindex != -1 and badindex > notindex:
        # можно вот так еще записать: if  -1 < notindex < badindex: 
        return s.replace(s[notindex:badindex+4], 'хорош')
    else:
        return s
...
def front_back(a, b):       # 2016.03.17_10:46:02 checked. prusanov
    # +++ ваш код +++
                # для читаемости - вынести (len(a)/2) и (len(b)/2) в отдельные переменные
                # попробуйте обойтись без преобразования типа int

...
def diff():         # 2016.03.23_21:31:24 checked. prusanov
    # +++ ваш код +++
    sum_of_sqrt = sum(list(map((lambda x: x**2), range(1,101))))        # можно не делать list()
    sqrt_of_sum = (sum(range(101)))**2
    return sqrt_of_sum-sum_of_sqrt      # без пробелов вокруг '-' плохо читаемо
...
def max_five():     # 2016.03.23_23:43:28 checked. prusanov
    # +++ ваш код +++
    list_of_sums=[]
    for i in range(len(max_five_string)-4):
        sum_of_five = reduce((lambda a,b: int(a)*int(b)), list(max_five_string[i:i+5]))     # можно не делать list()
...
def summm():        # 2016.03.23_23:44:38 checked. prusanov
    # +++ ваш код +++
    final_sum = reduce((lambda a,b: int(a)+int(b)), list(str(2**1000)))     # можно без list()
    return final_sum
...
def factorial():
    ...
    final_sum = reduce((lambda a,b: int(a)+int(b)), list(str(factorial)))       # можно без list()
    return final_sum

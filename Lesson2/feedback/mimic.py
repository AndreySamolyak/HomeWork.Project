...
def mimic_dict(filename):       # 2016.03.23_01:22:03 checked. prusanov
    ...
    dict = {}
    str = ''            # аккуратней с такими именами
    for word in words_list:     
        if str not in dict:     # лучше так:  if str not in dict:
            dict[str] = [word]
        else:
            if word not in dict[str]:       # эта проверка немного "сковывает" наш словарь, но это не страшно
                dict[str].append(word)
        str = word
    return dict


def print_mimic(mimic_dict, word):      # 2016.03.23_01:24:03 checked. prusanov
